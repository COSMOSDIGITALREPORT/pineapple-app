    ...
    <SafeAreaProvider>
      ...
      <SplashScreen onComplete={async () => {
        try {
          ...
          // Not logged in — always show onboarding, remember if safety was already accepted
          if (warningDone === 'true') {
            dispatch(acceptWarning());
      ...
      {screen === 'onboarding' &&
      <OnboardingScreen1
        onNext={() => warningAccepted ? setScreen('login') : setScreen('safety')}
        onLogin={() => warningAccepted ? setScreen('login') : setScreen('safety')} />
      }
